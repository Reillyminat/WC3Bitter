...
namespace WC3Binder
{
    public partial class Form1 : Form
    {
        ...
        const int WM_KEYDOWN = 0x0100; private const int WM_KEYUP = 0x0101;
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < 18; i++)
            {
                if (binder.newK[i].ToString() == "None")
                    Controls["button" + (i + 1)].Text = "";
                else
                    Controls["button" + (i + 1)].Text = binder.newK[i].ToString();
            }
            if (binder.GetExeLinks(name))
            {
            ...
            try
            {
                WindowHandle = Process.GetProcessesByName("Warcraft III")[0].MainWindowHandle;
            }
            catch (IndexOutOfRangeException)
            { };
            notifyIcon1.Text = "WC3Bitter";
        ...
        {
            if (nCode >= 0 && wParam == (IntPtr)WM_KEYUP && flag == true)
            {
                int vkCode = Marshal.ReadInt32(lParam);
                string ctext = ((Keys)vkCode).ToString();
                if (ctext == text)
                    flag = false;
            }
            if (nCode >= 0 && wParam == (IntPtr)WM_KEYDOWN && flag == false)
            {
                int vkCode = Marshal.ReadInt32(lParam);
                text = ((Keys)vkCode).ToString();
                if (timer != null)
                    timer.Dispose();
                timer = new System.Threading.Timer(tm, false, 0, 8000);
                if (text == textp && prevButton == true)
                {
                    selfCast = true;
                }
                prevButton = true;
                    ...
                    INPUT[] pInputs = new[] {new INPUT
                        {
                            ...
                            U = new InputUnion
                            {
                                ki = { wScan = 0, wVk = binder.code[number], dwFlags = 0 }
                            }
                        },new INPUT
                        {
                            type = InputType.KEYBOARD,
            ...
            else if (e.KeyCode >= Keys.A && e.KeyCode <= Keys.Z || e.KeyCode >= Keys.D1 && e.KeyCode <= Keys.D9 || e.KeyCode >= Keys.NumPad0 && e.KeyCode <= Keys.NumPad9 || e.KeyCode == Keys.XButton1 || e.KeyCode == Keys.XButton2)
            {
                for (int i = 1; i < 19; i++)
                    if (e.KeyCode.ToString() == this.Controls["button" + i.ToString()].Text)
                    {
                        ...
                        binder.setNewKey(i, Keys.None);
                    }
                this.Controls["button" + BN].Text = e.KeyCode.ToString();
                binder.setNewKey(Int32.Parse(BN), e.KeyCode);
                richTextBox1.Visible = false;
            }
            this.Controls["button" + BN].BackColor = SystemColors.Control;
        }
        private void SetKeyBackColor(string bt)
        {
            ...
            richTextBox1.Focus();
            BN = bt;
            this.Controls["button" + BN].BackColor = Color.Bisque;
        }
        ...
        private void button19_Click(object sender, EventArgs e)
        {
            try
            {
                if (richTextBox2.Text != "")
                {
                    ...
                    if (checkBox1.Checked)
                        argc = "-nativefullscr";
                    Process.Start(richTextBox2.Text, argc);
                }
                try
                {
                    Thread.Sleep(4000);
            ...
            {
                MessageBox.Show("Не удаётся найти указанный файл", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            };
        }
        private void button20_Click(object sender, EventArgs e)
        {
            ...
            {
                if (richTextBox2.Text != "")
                    Process.Start(richTextBox3.Text);
                try
                {
                    WindowHandle = Process.GetProcessesByName("Warcraft III")[0].MainWindowHandle;
                }
